...
object RsaCipherHelper {

    private const val TAG = "RsaCipherHelper"
    ...
    internal fun init(applicationContext: Context) {
        if (isSupported) {
            Log.d(TAG,"Already initialised - Do not attempt to initialise this twice")
            return
        }
        ...
        result = if (keyStore.containsAlias(alias)) {
            ...
            Log.d(TAG,"No keypair for %s, creating a new one")
    ...
    private fun initAndroidM(alias: String): Boolean {
        try {
            ...
            Log.d(TAG,"Random keypair with ${KeyProperties.KEY_ALGORITHM_RSA}/${KeyProperties.BLOCK_MODE_CBC}/${KeyProperties.ENCRYPTION_PADDING_RSA_PKCS1} is created.")

            return true
        } catch (e: GeneralSecurityException) {
            Log.d(TAG, "It seems that this device does not support RSA algorithm!!")
    ...
    private fun initAndroidL(alias: String): Boolean {
        try {
            ...
            Log.d(TAG,"Random RSA algorithm keypair is created.")

            return true
        } catch (e: GeneralSecurityException) {
            Log.d(TAG, "It seems that this device does not support encryption!!")
